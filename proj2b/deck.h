...
public:
    ...
    ~deck();                            // destructor: de-allocate deck of cards
    // gameplay functions
    card deal();                        // deal top card
    void replace(card& oldCard);        // replace card on bottom of deck
...
/*
 * Destructor to deallocate memory for all deck nodes
 */
deck::~deck()
{
    node<card> * tempCard;
...
/*
 * Deal the top card from deck and return it - resets firstCard in deck
 */
card deck::deal()
{
    card topCard;

    // deal top card as first card on deck
    topCard = firstCard->nodeValue;
    // if current card is top card
    if (currCard == firstCard)  { currCard = firstCard->next; }
    // set top card to second card in deck (new top card)
    firstCard = firstCard->next;

    // return (former) first card in deck
    return topCard;
}


/*
 * Given card ptr, card will be inserted at the bottom of the deck.
 */
void deck::replace(card& oldCard)
{
    node<card> * bottomCard;
    // turn passed card into a node to insert into list
    bottomCard = new node<card> (oldCard, NULL);

    // start searching at front of deck
    currCard = firstCard;
    // move current position to last card
    while (currCard->next != NULL)  { currCard = currCard->next; }

    // insert passed card
    currCard->next = bottomCard;
}
